# Borrowed from : https://github.com/iamgroot42/cleverhans/blob/future/pytorch/hsja/cleverhans/future/torch/attacks/hop_skip_jump_attack.py
# This is my own implementation (ported from the TF version of the original cleverhans repository)
...
def hop_skip_jump_attack(model_fn, x, norm,
    ...
    def hsja(sample, target_label, target_image):
      if target_label is None:
        _, original_label = ch.max(model_fn(sample), 1)
      ...
      def decision_function(images):
        ...
        0 otherwise.
        """
        images = ch.clamp(images, clip_min, clip_max)
        prob = []
        for i in range(0, len(images), batch_size):
          batch = images[i:i+batch_size]
          prob_i = model_fn(batch)
          prob.append(prob_i)
        prob = ch.cat(prob, dim=0)
        if target_label is None:
          return ch.max(prob, dim=1)[1] != original_label
        else:
          return ch.max(prob, dim=1)[1] == target_label
      ...
      for j in np.arange(num_iterations):
        ...
        if stepsize_search == 'geometric_progression':
          ...
          # Update the sample.
          perturbed = ch.clamp(perturbed + epsilon * update,
                                  clip_min, clip_max)
          ...
          epsilons = ch.from_numpy(np.logspace(-4, 0, num=20, endpoint=True)).to(perturbed.device).float() * dist
          perturbeds = perturbed + epsilons.view((20,) + (1,) * (len(shape) - 1)) * update
          perturbeds = ch.clamp(perturbeds, clip_min, clip_max)
          idx_perturbed = decision_function(perturbeds)

          if ch.sum(idx_perturbed) > 0:
    ...
    for i, x_ in enumerate(x):
      ...
      adv_x.append(pert)
    return ch.cat(adv_x, 0)

def compute_distance(x_ori, x_pert, constraint=2):
  """ Compute the distance between two images. """
  if constraint == 2:
    dist = ch.norm(x_ori - x_pert, p=2)
  elif constraint == np.inf:
    dist = ch.max(ch.abs(x_ori - x_pert))
  return dist
...
def approximate_gradient(decision_function, sample, num_evals,
  ...
  if constraint == 2:
    rv = ch.randn(noise_shape)
  elif constraint == np.inf:
    rv = -1 + ch.rand(noise_shape) * 2

  axis = tuple(range(1, 1 + len(shape)))
  rv = rv / ch.sqrt(ch.sum(rv ** 2, dim=axis, keepdim=True))
  perturbed = sample + delta * rv.to(sample.device)
  perturbed = ch.clamp(perturbed, clip_min, clip_max)
  ...
  fval_mean = ch.mean(fval)
  if fval_mean == 1.0:  # label changes.
    gradf = ch.mean(rv, dim=0)
  elif fval_mean == -1.0:  # label not change.
    gradf = - ch.mean(rv, dim=0)
  else:
    fval = fval - fval_mean
    gradf = ch.mean(fval * rv, dim=0)

  # Get the gradient direction.
  gradf = gradf / ch.norm(gradf, p=2)
...
def project(original_image, perturbed_images, alphas, shape, constraint):
  ...
  if constraint == 2:
    ...
    projected = ch.clamp(perturbed_images,
...
def binary_search_batch(original_image, perturbed_images, decision_function,
  ...
  if constraint == np.inf:
    ...
    # Stopping criteria.
    thresholds = ch.min(dists_post_update * theta, theta)
  else:
    highs = ch.ones(len(perturbed_images)).to(original_image.device)
    thresholds = theta

  lows = ch.zeros(len(perturbed_images)).to(original_image.device)

  while ch.max((highs - lows) / thresholds) > 1:
    # projection to mids.
    mids = (highs + lows) / 2.0
    ...
    lows  = ch.where(decisions == 0, mids, lows)
    highs = ch.where(decisions == 1, mids, highs)
...
def initialize(decision_function, sample, shape, clip_min, clip_max):
  ...
  while True:
    random_noise = clip_min + ch.rand(shape).to(sample.device) * (clip_max - clip_min)
